...
function generateWorkout(triggerEmail) {
  ...
  const userHeaders = (dataUserProfile && dataUserProfile[0])
    ? dataUserProfile[0].map(h => String(h || "").trim())
    : [];

  const idxOfHeader_ = (headers, names) => {
  ...
  const normalizeProgramType_ = (raw, selectedType) => {
    const v = String(raw || "").trim().toLowerCase();
    if (v === "strength" || v === "force" || v === "musculation") return "Strength";
    if (v === "hiit") return "HIIT";
    if (v === "yoga") return "Yoga";
    if (v === "mobility" || v === "mobilite" || v === "mobilité") return "Mobility";
    if (v === "pilates" || v === "wallpilates" || v === "wall pilates") return "Pilates";

    // Fallback: déduction depuis le type de séance
    const t = String(selectedType || "").toLowerCase();
    if (t.includes("pilates")) return "Pilates";
    if (t.includes("yoga")) return "Yoga";
    if (t.includes("mobility") || t.includes("mobilite") || t.includes("mobilité")) return "Mobility";
  ...
  const programTypeIdx = idxOfHeader_(userHeaders, ["ProgramType", "Program Type", "Programme", "Discipline"]);
  const programType = normalizeProgramType_(programTypeIdx !== -1 ? userRowData[programTypeIdx] : "", selectedType);
...
function pickRandomExercise(code, dbData, dbHeaders, userEquipList, userConstraints) {
  ...
  const IDX_DISCIPLINE_LOCAL = idxOfLocal(["discipline", "programtype", "program_type"]);
  ...
  // --- B2. FILTRE PROGRAMTYPE / DISCIPLINE (si la colonne existe et est renseignée) ---
  const requestedProgramType = String((userConstraints && userConstraints.programType) || "").trim().toLowerCase();
  if (requestedProgramType && IDX_DISCIPLINE_LOCAL !== -1) {
    const matching = dbData.filter(row => {
      const d = String(row[IDX_DISCIPLINE_LOCAL] || "").trim().toLowerCase();
      return d !== "" && d === requestedProgramType;
    });
    // On applique le filtre seulement si au moins quelques exos sont taggés avec cette discipline
