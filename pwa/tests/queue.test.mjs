...
function getPendingBatchCount() {
  try { const arr = JSON.parse(localStorage.getItem('fitbook_pending_batches') || '[]'); return Array.isArray(arr) ? arr.length : 0 } catch { return 0 }
}
...
beforeEach(() => {
  ...
  localStorage.removeItem('fitbook_pending_batches')
})

test('indicator hidden when no pending batches', () => {
  localStorage.setItem('fitbook_pending_batches', JSON.stringify([]))
...
test('indicator shows count when pending batches present', () => {
  localStorage.setItem('fitbook_pending_batches', JSON.stringify([{id:1},{id:2}]))
  updateQueueIndicator()
  const el = document.getElementById('queue-indicator')
  const cnt = document.getElementById('queue-count')
